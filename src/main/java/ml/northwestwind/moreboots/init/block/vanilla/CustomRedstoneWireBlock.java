...
public class CustomRedstoneWireBlock extends Block {
    ...
    public void onEntityCollision(BlockState state, World worldIn, BlockPos pos, Entity entityIn) {
        if (entityIn instanceof LivingEntity && ((LivingEntity) entityIn).getItemStackFromSlot(EquipmentSlotType.FEET).getItem().equals(ItemInit.REDSTONE_BOOTS)) {
            this.canProvidePower = true;
            worldIn.setBlockState(pos, state.with(POWER, 15), 3);
            updateDiagonalNeighbors(state, worldIn, pos, 3, 0);
