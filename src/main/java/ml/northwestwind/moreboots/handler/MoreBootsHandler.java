...
import ml.northwestwind.moreboots.init.block.InvisibleBlock;
...
import static ml.northwestwind.moreboots.init.item.ArmorItemBase.breakGlassBoots;
...
public class MoreBootsHandler {
    ...
    public void onLivingUpdate(final LivingEvent.LivingUpdateEvent event) {
        ...
        if (boots.getItem().equals(ItemInit.ICE_BOOTS)) {
            ...
            if (underneath.isSolid() && entity.world.isAirBlock(blockPos) && rng.nextInt(100) == 0) {
            ...
            List<Entity> entities = entity.world.getEntitiesInAABBexcluding(entity, new AxisAlignedBB(new BlockPos(entity.getPositionVec())).grow(5), EntityPredicates.NOT_SPECTATING);
            ...
            for (Entity ent : entities) {
                ...
                if (player.addItemStackToInventory(((ItemEntity) ent).getItem())) available--;
            }
        } else if (boots.getItem().equals(ItemInit.GLIDER) && (entity.getMotion().getX() != 0 || entity.getMotion().getZ() != 0)) {
            if (entity.getMotion().getY() < 0.02) {
    ...
    public void onKeyInput(final InputEvent.KeyInputEvent event) {
        if (KeybindInit.teleport.isPressed()) {
        ...
        } else if (event.getKey() == KeybindInit.openStorage.getKey().getKeyCode()) {
            Minecraft minecraft = Minecraft.getInstance();
            if(minecraft.currentScreen instanceof StorageBootsScreen)
            {
            ...
            else if(minecraft.player != null && minecraft.currentScreen == null)
            {
                ...
                if(KeybindInit.openStorage.isPressed())
                {
                    if(player.getItemStackFromSlot(EquipmentSlotType.FEET).getItem() instanceof StorageBootsItem)
                    {
                        MoreBootsPacketHandler.INSTANCE.sendToServer(new COpenStorageBootsPacket());
                        minecraft.getSoundHandler().play(SimpleSound.master(SoundEvents.BLOCK_CHEST_OPEN, 0.75F, 1.0F));
                    }
                }
            }
        }
    ...
    public static void preRenderPlayer(final RenderPlayerEvent.Pre event) {
        GlStateManager.blendFunc(GlStateManager.SourceFactor.ONE.param, GlStateManager.DestFactor.ONE_MINUS_SRC_ALPHA.param);
    ...
    public static void preRenderLiving(final RenderLivingEvent.Pre event) {
        if (event.getEntity() instanceof PlayerEntity) {
            if (event.isCancelable() && event.getEntity().getItemStackFromSlot(EquipmentSlotType.FEET).getItem().equals(ItemInit.LOKI_BOOTS)) {
                    event.setCanceled(true);
            }
        }
    }
